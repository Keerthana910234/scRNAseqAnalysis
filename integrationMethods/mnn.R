...
geneList <- Reduce(intersect, list(Features(naive2Filtered), Features(SotAFiltered), Features(GemFiltered)))

naive2FilteredSubset <- preprocessSeurat(subset(naive2Filtered, features = geneList))
SotAFilteredSubset <- preprocessSeurat(subset(SotAFiltered, features = geneList))
GemFilteredSubset <- preprocessSeurat(subset(GemFiltered, features = geneList))

naive <- as.SingleCellExperiment(naive2FilteredSubset)
sot <- as.SingleCellExperiment(SotAFilteredSubset)
gem <- as.SingleCellExperiment(GemFilteredSubset)

library(batchelor)
classic.out <- fastMNN(naive, sot, gem, subset.row = geneList)
...
combinedSeuratObjectPreprocessed <- readRDS("/projects/b1042/GoyalLab/Keerthana/subiaHanxiaoDataAnalysis/extractedData/rdsObjects/combinedSeuratObjectPreprocessed.rds")

# integratedObject <- RunHarmony(combinedSeuratObjectPreprocessed, c("orig.ident"), plot_convergence = T)
integratedObject <- IntegrateLayers(object = combinedSeuratObjectPreprocessed, method = FastMNNIntegration, 
                                    new.reduction = 'fastMNN', features = Features(combinedSeuratObjectPreprocessed))
