...
pathToFiles = "/projects/b1042/GoyalLab/Keerthana/subiaHanxiaoDataAnalysis/extractedData/bulkRNAseq/outputNew/"
...
counttablefull <- counttablefull %>%
  mutate(ensembl_gene =gene_SA_MP2_E7_naive_1_S1) %>%
...
write_csv(counttablefull, "/projects/b1042/GoyalLab/Keerthana/subiaHanxiaoDataAnalysis/extractedData/BulkRNAseq_countTableAll.csv")
counttablefull <- read_csv("/projects/b1042/GoyalLab/Keerthana/subiaHanxiaoDataAnalysis/extractedData/BulkRNAseq_countTableAll.csv")
...
columnDataForDeDeq <- data.frame(sample = colnames(bulkSeqData))
...
dds <- DESeqDataSetFromMatrix(countData = bulkSeqData,
...
res_naive_vs_sotarasib <- results(dds, contrast=c("condition", "Naive", "Sotorasib"))
res_naive_vs_sotarasib <- data.frame(res_naive_vs_sotarasib)
...
all_results_heatmap <- all_results_name %>%
